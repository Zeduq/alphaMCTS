...
if __name__ == "__main__":
    # 解析结果文件
    new_factors = parse_result_file('results/logs/result.txt')
    
    print(f"新版本因子数量: {len(new_factors)}")
    
    # 旧版本对比（如存在）
    old_path = 'results/logs/result_old.txt'
    if os.path.exists(old_path):
        old_factors = parse_result_file(old_path)
        ...
        print("旧版本结果文件不存在，跳过新旧对比。")
        old_factors = []
        combined_df = pd.DataFrame(new_factors)
