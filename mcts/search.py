...
def is_formula_valid(formula: AlphaFormula) -> bool:
    ...
    for step in formula.formula_steps:
        ...
        if op_name in binary_ops and len(inputs) != 2:
            print(f"校验失败: 发现无效的二元运算。操作符 '{op_name}' 有 {len(inputs)} 个输入，应为2个。")
            return False
    return True
...
class MCTS:
    ...
    # [主要修改点 1]: 在 expand 方法的参数中增加 alpha_repo
    def expand(self, node_to_expand: AlphaNode, freq_subtrees: List[str], alpha_repo: AlphaLibrary) -> Optional[AlphaNode]:
        """
        ...
        # [主要修改点 2]: 将 alpha_repo 传递给 simulate_evaluation
