...
任务目标：
1. 支持1日/5日/10日/20日四个持仓周期的回测
2. 对比Baseline（单体LLM）和本研究方法（多智能体辩论）的因子表现
3. 计算Rank IC、Raw IR、净IR、日均换手率、IC半衰期、信息留存率
...
class BaselineMCTS(MCTS):
    """Baseline MCTS：使用Refiner Agent替代多智能体辩论"""
...
def run_single_experiment(method: str, factor_type: str, target_count: int = 5) -> List[AlphaNode]:
    ...
    Args:
        method: "baseline" 或 "debate"
        factor_type: 因子类型（动量/价值/波动率）
        target_count: 目标因子数量
