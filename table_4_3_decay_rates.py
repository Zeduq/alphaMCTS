"""
表4-3 不同持仓周期的因子各指标衰减率计算
Table 4-3 Decay Rates of Factor Metrics Across Different Holding Periods

基于4_2_graph.py生成的数据，计算各指标相对于1日周期的衰减率
"""
...
for period in holding_periods:
    for i, method in enumerate(methods):
        ...
        if not period_data.empty:
            ...
            # 1日周期不显示衰减率
            if period == 1:
                latex_code += f"{period}日 & {method_name} & {ir:.4f} & {net_ir:.4f} & {turnover:.4f} \\\\\n"
            else:
                latex_code += f"{period}日 & {method_name} & {ir:.4f} ({ir_decay_rates[period]:.1f}\\%) & {net_ir:.4f} ({net_ir_decay_rates[period]:.1f}\\%) & {turnover:.4f} ({turnover_decay_rates[period]:.1f}\\%) \\\\\n"
...
# 保存LaTeX代码
latex_path = os.path.join(OUTPUT_DIR, 'table_4_3_latex.tex')
...
for method in methods:
    ...
    print(f"\n【{method_name}】")
    print(f"  IR衰减: 1日→20日 衰减 {ir_decay_rates[20]:.1f}%")
    print(f"  Net IR衰减: 1日→20日 衰减 {net_ir_decay_rates[20]:.1f}%")
    
    # 计算衰减速度对比
    if method == 'Ours':
        baseline_ir_decay = calculate_decay_rate(df_summary, 'Baseline', 'IR')[20]
        ours_ir_decay = ir_decay_rates[20]
        improvement = baseline_ir_decay - ours_ir_decay
        print(f"  → 相比Baseline，IR衰减减缓 {improvement:.1f} 个百分点")
